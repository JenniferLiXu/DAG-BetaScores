...
partitionMCMC_betas<-function(n,startpermy,startparty,iterations,stepsave,betas,moveprobs){
  
  newedgerevallowed<-0 # do not allow the new edge reversal moves
  ...
  currenttotallogscore<-sum(currentpartitionscores) #log total score of all DAGs in the starting partition and permutation
  
  currentDAG<-samplescore_partition(n, betas, currentpermy, currentparty,currentposy, base_score) #log score of a single sampled DAG from the partition and permutation
  ...
  for (z in 2:zlimit){ #the MCMC chain loop with 'iteration' steps is in two parts
    for (count in 1:stepsave){ #since we only save the results to the lists each 'stepsave'
      ...
      if(chosenmove<3){	# if it is <3 then we swap two elements
        ...
        if(length(currentparty)>1){ # if the partition only has one element then we cannot move
          ...
          proposedpartitionrescored<-partitionscore(n,rescorenodes,betas,proposedpermy,currentparty,currentposy) #their scores
          proposedtotallogscore<-currenttotallogscore-sum(currentpartitionscores[rescorenodes])+sum(proposedpartitionrescored[rescorenodes]) #and the new log total score by updating only the necessary nodes
          ...
          if(runif(1)<scoreratio){ #Move accepted then set the current permutation and scores to the proposal
            ...
            currentpartitionscores[rescorenodes]<-proposedpartitionrescored[rescorenodes]
        ...
        proposedpartitionrescored<-partitionscore(n,rescorenodes,betas,proposedpermy,proposedparty,proposedposy) #only rescore the necessary nodes
        proposedtotallogscore<-currenttotallogscore-sum(currentpartitionscores[rescorenodes])+sum(proposedpartitionrescored[rescorenodes]) #and calculate the new log total score by updating only the necessary nodes
      ...
      #   currentDAG<-samplescore_partition(n, betas, permy, party,base_score) # sample a DAG
      ...
      #     
      #     # rescore everything
      #     currentpartitionscores<-partitionscore(n,c(1:n),betas,currentpermy,currentparty,currentposy) #scores etc of all DAGs compatible with the current permutation and partition
      #     currenttotallogscore<-sum(currentpartitionscores) #log total score
      #     
    ...
    currentDAG<-samplescore_partition(n, betas, currentpermy, currentparty, currentposy,base_score)
